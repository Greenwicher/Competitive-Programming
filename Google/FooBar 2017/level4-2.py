...
def cycle(a, b):
    """
    check if two guards with bananas a and b can play forever
    idea: enumerate possible (a,b) from the ending point ((a+b)/2, (a+b)/2) backwards, and find the formula
    example: (k,k) -> (k/2, 3k/2), (k/2, 3k/2) -> (k/4, 7k/4) or (5k/4, 3k/4) and so on and so forth, where k = (a+b)/2
    :param a: int, the number of banana of guard A
    :param b: int, the number of banana of guard B
    :return: Bool, whether they can play forever
    """
    divisor = gcd(a, b)
    num = (a + b) // divisor
    # check whether num is not a power of two
    ans = not (((num & (num-1)) == 0) and num != 0)
    return ans

def answer(banana_list):
    """
    Firstly, build the graph where vertex i and vertex j are connected if they can play forever (i.e. cycle(banana_list[
    i], banana_list[j]) is True)
    Secondly, apply the Edmonds' Blossom algorithm to find the maximum matching given the general graph
    Finally, count the number of guards / vertices that cannot be matched to any other guards
    :param banana_list:
    :return: the fewest possible number of guards that will be left to watch the prisoners
    """
    n = len(banana_list)
    edges, alone = [], [] # alone stores the index of guards that cannot play forever with all the other guards
    for i in range(n):
        flag = True
        for j in range(n):
            # check whether i and j can play forever
            if cycle(banana_list[i], banana_list[j]):
                edges.append((i, j, 1))
                flag = False
        if flag:
            alone.append(i)
    # find the maximum matching based on Blossom algorithm, mate[i] = j, if i and j are matched, mate[i] = -1, if i
    # cannot be matched with any others
    mate = maxWeightMatching(edges, maxcardinality=True)
    return sum([m == -1 for m in mate]) + len(alone)

print(answer([1,1]),2)
print(answer([1, 7, 3, 21, 13, 19]),0)
print(answer([1,1,4,1,2,1073741823]))