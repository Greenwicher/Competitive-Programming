...
import math
def enumeratePath(n, m, N, M):
    #assert N - n > M - m, "%d,%d" % (N - n, M - m)
    if m == 0: return math.factorial(n)
    if n == 0: return math.factorial(m)
    A = N - n + 1 > M - m
    B = N - n > M - m + 1
    if A and B:
        return n * enumeratePath(n-1, m, N, M) + m * enumeratePath(n, m-1, N, M)
    if A and not(B):
        return n * enumeratePath(n-1, m, N, M)
    if not(A) and B:
        return m * enumeratePath(n, m-1, N, M)
    if not(A) and not(B):
        return 0
    return
...
def solve(N, M):
    ...
    count = enumeratePath(N, M, N, M)
    ans = count / math.factorial(N+M)
    timing.append(time.time())
    return ans
