...
def bitwiseExpectation(N, X, K, A, B, C):
    binx, bink = bin(X)[2:], bin(K)[2:]
    lx, lk = len(binx), len(bink)
    length = max(lx, lk)
    binx, bink = '0' * (length - lx) + binx, '0' * (length - lk) + bink
    ans = 0
    for i in range(length):
        x, k = binx[length - i - 1], bink[length - i - 1]
        p00, p01, p10, p11 = 1 * (x == '0'), 1 * (x == '1'), 0, 0
        if k == '0':
            for _ in range(N):
                p10 = 1 * p00 + A / 100 * p01
                p11 = 0 * p00 + (B + C) / 100 * p01
                p00, p01, p10, p11 = p10, p11, 0, 0
        else:
            for _ in range(N):
                p10 = A / 100 * p00 + C / 100 * p01
                p11 = (B + C) / 100 * p00 + (A + B) / 100 * p01
                p00, p01, p10, p11 = p10, p11, 0, 0
        ans += p01 * 2 ** i
    return ans
...
def solve(N, X, K, A, B, C):
    ...
    ans = bitwiseExpectation(N,X,K,A,B,C)
    timing.append(time.time())
    return ans
